...
export async function listStudents(token: string, search?: string) {
  ...
  return apiFetch<{ data: Student[] }>(`/students${query}`, { token });
}
...
export async function updateStudentStatus(token: string, id: number, status: string, reason?: string) {
...
export async function enrolStudent(
  ...
  return apiFetch<Student>(`/students/${id}/enrolments`, {
    method: "POST",
    token,
    body: JSON.stringify({ academic_session_id: academicSessionId, level_id: levelId }),
  });
}
...
export async function transferStudentProgramme(
  ...
  return apiFetch<Student>(`/students/${id}/transfer`, {
    method: "POST",
    token,
    body: JSON.stringify({ to_programme_id: toProgrammeId, reason }),
  });
}

export async function getMyStudentRecord(token: string) {
  return apiFetch<Student>("/student/me", { token });
}
