...
export type StaffResultListData = {
  items: Result[];
...
export async function getRoster(token: string, offeringId: number) {
  return apiFetch<RosterStudent[]>(
    `/lecturer/courses/${offeringId}/roster`,
    { token }
  );
}

export async function listLecturerResults(
  token: string,
  offeringId: number
) {
  return apiFetch<Result[]>(
    `/lecturer/courses/${offeringId}/results`,
    { token }
  );
}
...
export async function submitResults(
  ...
  return apiFetch<Result[]>(
    `/lecturer/courses/${offeringId}/results/submit`,
    {
      method: "POST",
      token,
    }
...
export async function listStaffResults(
  token: string,
  status?: string
) {
  const query = status
  ...
  return apiFetch<StaffResultListData>(
    `/staff/results${query}`,
    { token }
  );
}
