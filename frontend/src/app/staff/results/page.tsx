...
export default async function StaffResultsPage() {
  ...
  if (!session || !can(session, "results.view")) {
    return (
      <Container className="py-16">
        <EmptyState title="You don't have access to this page" description="Sign in with an HOD or Academic Officer account." />
      </Container>
    );
  }
  ...
  const results: Result[] = body.success
    ? ((body.data as unknown as { data: Result[] }).data ?? (body.data as unknown as Result[]))
    : [];

  return (
    <Container className="py-12">
      <h1 className="font-[family-name:var(--font-display)] text-2xl text-ink">Results Pipeline</h1>
      <p className="mt-1 text-sm text-muted">
        SUBMITTED → REVIEWED (HOD) → VERIFIED → APPROVED → PUBLISHED (Academic Officer).
      ...
      <div className="mt-8">
        {results.length === 0 ? (
          <EmptyState title="Nothing pending" description="Results awaiting review, verification, approval, or publication will appear here." />
        ) : (
          <ResultQueueTable
