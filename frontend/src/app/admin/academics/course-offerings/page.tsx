...
import { getAdminCourseOfferings, getAdminCourses, getAdminSessions, getAdminSemesters, getAdminLevels } from "@/lib/api/admin-academics";
...
export default async function AdminCourseOfferingsPage() {
  ...
  const [offeringsResult, coursesResult, sessionsResult, semestersResult, levelsResult, programmesResult] = await Promise.all([
    ...
    <Container className="flex flex-col gap-6 py-12">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          Lecturer is assigned by numeric user ID for now — a proper staff picker will follow once a
          dedicated staff-listing endpoint exists for it (see docs/PROJECT_STATUS.md).
        </p>
      </div>
      ...
      <AcademicEntityForm
        ...
        fields={[
          ...
          { name: "lecturer_id", label: "Lecturer (user ID, optional)", type: "number" },
